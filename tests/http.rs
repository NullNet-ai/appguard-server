...
use crate::rpc::{handle_http_request, handle_http_response, handle_tcp_connection};
...
async fn test_grpc_server_durable_storage() {
    ...
    for i in 1..=NUM_ITER {
        // handle_http_request
        let ret_val = handle_http_request(&mut client, sample_http_request(i, true))
            .await
            .unwrap();
        assert_eq!(ret_val.policy, FirewallPolicy::Allow as i32);

        // handle_http_response
        let ret_val = handle_http_response(&mut client, sample_http_response(0, true))
            .await
            .unwrap();
        assert_eq!(ret_val.policy, FirewallPolicy::Allow as i32);
    }
...
async fn test_grpc_server_durable_storage_with_more_data_and_then_expire() {
    ...
    for i in 1..=NUM_ITER * 2 {
        ...
        if i % 2 == 0 {
            ...
            let ret_val = handle_http_request(&mut client, http_request)
                .await
                .unwrap();
            assert_eq!(ret_val.policy, FirewallPolicy::Allow as i32);
        } else {
        ...
        if i % 2 == 0 {
            ...
            let ret_val = handle_http_response(&mut client, http_response)
                .await
                .unwrap();
            assert_eq!(ret_val.policy, FirewallPolicy::Allow as i32);
        } else {
...
async fn test_grpc_server_durable_storage_with_empty_data() {
    ...
    for _ in 1..=NUM_ITER {
        // handle_http_request
        let ret_val = handle_http_request(&mut client, AppGuardHttpRequest::default())
            .await
            .unwrap();
        assert_eq!(ret_val.policy, FirewallPolicy::Allow as i32);

        // handle_http_response
        let ret_val = handle_http_response(&mut client, AppGuardHttpResponse::default())
            .await
            .unwrap();
        assert_eq!(ret_val.policy, FirewallPolicy::Allow as i32);
    }
...
async fn test_grpc_server_with_log_requests_disabled() {
    ...
    for _ in 1..=NUM_ITER {
        // handle_http_request
        handle_http_request(&mut client, AppGuardHttpRequest::default())
            .await
            .unwrap();

        // handle_http_response
        handle_http_response(&mut client, AppGuardHttpResponse::default())
            .await
            .unwrap();
    }
...
async fn test_grpc_server_http_response_time() {
    ...
    for _ in 1..=NUM_ITER {
        ...
        let ret_val = handle_tcp_connection(&mut client, sample_tcp_connection("HTTP".to_string()))
            .await
            .unwrap();
        let tcp_id = ret_val.tcp_info.as_ref().unwrap().tcp_id;

        // handle_http_request
        handle_http_request(&mut client, sample_http_request(tcp_id, true))
            .await
            .unwrap();

        // handle_http_response
        handle_http_response(&mut client, sample_http_response(tcp_id, true))
            .await
            .unwrap();
    }
...
async fn test_grpc_server_http_request_ai_dataset() {
    ...
    for _ in 1..=NUM_ITER {
        // handle_tcp_connection
        let ret_val =
            handle_tcp_connection(&mut client, sample_tcp_connection_2("HTTP".to_string()))
                .await
                .unwrap();
        let tcp_id = ret_val.tcp_info.as_ref().unwrap().tcp_id;

        // handle_http_request
        handle_http_request(&mut client, sample_http_request_2(tcp_id, true))
            .await
            .unwrap();
    }
